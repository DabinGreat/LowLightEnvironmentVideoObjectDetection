...
CATE = {'1': 'person', '2': 'vehicle'}
IMAGE_ROOT = '/data/PANDA/'
FASTER_RCNN_CONFIG = '/home/dabingreat666/code/detect/mmdetection/configs/_panda_round1/'  # model configs
CKPT_PATH = '/home/dabingreat666/code/detect/mmdetection/checkpoints/cascade_rcnn_r101_fpn_dconv/'  # model checkpoints
RESULT_PATH = '/home/dabingreat666/code/detect/mmdetection/test_result'  # save the detection results

# IMAGE_ROOT = '/data/panda/'
# FASTER_RCNN_CONFIG = './configs/_panda_round1/'  # model configs
# CKPT_PATH = './checkpoints/cascade_rcnn_r101_fpn_dconv/'  # model checkpoints
# RESULT_PATH = './test_result'  # save the detection results
SCORE_THRES = 0.05

# TODO try to use various combinations of WIDTH and HEIGHT, for example:(1000,500),(2000,1000)...
HEIGHT = 2000  # Sliding Window Height
WIDTH = 2000  # Sliding Window Width
...
def nms_after_det(bboxes_list,
    ...
    for i, bbox in enumerate(bboxes_list):
        ...
        if 0 <= labels_list[i] <= 7:  # choose what to keep, now keep person and all vehicles
            multi_bboxes.append(bbox[0:4])
            temp = [0 for _ in range(len(class_names))]
            # temp = [0 for _ in range(3)]
            temp[labels_list[i]] = bbox[4]
...
def main(cate_id, faster_rcnn_url, faster_rcnn_cfg, result_path):
    ...
    img_paths = []
    img_ids = []
    cfg, model = detector_prepare(faster_rcnn_url, cfg_path=faster_rcnn_cfg)
    ...
    for i, img_path in enumerate(img_paths):
        ...
        det_bboxes, det_labels = nms_after_det(bboxes_list, labels_list, cfg_path=faster_rcnn_cfg)

        for idx in range(det_bboxes.shape[0]):
            item = {
                "image_id": int(img_ids[i]),
                # "category_id": int(cate_id),
                "category_id": det_labels[idx] + int(cate_id),
...
if __name__ == '__main__':
    ...
    for i in range(1, 3):
        ...
        faster_rcnn_cfg = pt.join(FASTER_RCNN_CONFIG, 'cascade_rcnn_r101_fpn_dconv_1x_panda_{}_1.py'.format(CATE[cate_id]))
        main(cate_id, faster_rcnn_url, faster_rcnn_cfg, result_path)
