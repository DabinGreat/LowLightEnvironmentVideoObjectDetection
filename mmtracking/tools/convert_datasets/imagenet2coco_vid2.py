...
def parse_val_list(ann_dir):
    ...
    img_list = osp.join(ann_dir, 'Lists/VID_val_videos_30.txt')
    img_list = mmcv.list_from_file(img_list)
    val_infos = defaultdict(list)
    video_name = []
    for info in img_list:
        info = info.split(' ')
        if info[0] not in video_name:
            val_infos[info[0]] = [int(info[2])]
            video_name.append(info[0])
        else:
            val_infos[info[0]].append(int(info[2]))
    return val_infos
...
def convert_vid(VID, ann_dir, save_dir, mode='train'):
    ...
    for vid_info in tqdm(vid_infos):
        ...
        for frame_id in vid_infos[vid_info]:
            ...
            records['img_id'] += 1
        records['vid_id'] += 1
    mmcv.dump(VID, osp.join(save_dir, f'imagenet_vid_30_{mode}.json'))
