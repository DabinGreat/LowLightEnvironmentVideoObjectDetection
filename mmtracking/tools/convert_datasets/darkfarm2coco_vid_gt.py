...
def parse_train_list(ann_dir):
    img_list = osp.join(ann_dir, 'Lists/darkfarm_vid_gt_train.txt')
...
def convert_darkfarm(VID, ann_dir, save_dir, mode='train', prefix=''):
    ...
    for vid_info in tqdm(vid_infos):
        ...
        for frame_id in range(num_frames):
            ...
            xml_name = osp.join(ann_dir, vid_info, '%d.xml' % frame_id)
...
def main():
    ...
    for k, v in enumerate(CLASSES, 1):
        ...
        txt_to_be = ['darkfarm_vid_gt_train.txt', 'darkfarm_vid_gt_val.txt']
