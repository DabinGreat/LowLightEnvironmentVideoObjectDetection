    ...
    def ref_img_sampling(self,
        ...
        if (not self.load_as_video) or img_info.get('frame_id', -1) < 0 \
            ...
            if method == 'uniform':
                ...
                if filter_key_img and img_id in valid_ids and len(valid_ids) > 1:
                    valid_ids.remove(img_id)
                if keep_samples_length and num_ref_imgs > len(valid_ids):
                    ks = num_ref_imgs - len(valid_ids)
                    repeat_ids = random.choices(valid_ids, k=ks)
                    ref_img_ids.extend(sorted(valid_ids + repeat_ids))
                else:
                ...
                for mode in ['left', 'right']:
                    ...
                    if filter_key_img and img_id in valid_ids and len(valid_ids) > 1:
                        valid_ids.remove(img_id)
                    if keep_samples_length and (num_ref_imgs // 2) > len(valid_ids):
                        ks = (num_ref_imgs // 2) - len(valid_ids)
                        repeat_ids = random.choices(valid_ids, k=ks)
                        ref_img_ids.extend(sorted(valid_ids + repeat_ids))
                    else:
