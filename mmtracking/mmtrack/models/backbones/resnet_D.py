...
class LayerDenoisingPlugin(nn.Module):
    ...
    def temporal_fusion(self, x):
        t, c, h, w = x.size()
        feat = []
        plot_feats(x, './plot/input.jpg')
        for i in range(t):
            x_ref = x[[i]]
            plot_feats(x_ref.repeat(t, 1, 1, 1), './plot/x_ref.jpg')
            x_cat = torch.cat([x, x_ref.repeat(t, 1, 1, 1)], dim=1)
            x_set = self.offset_conv(x_cat)
            plot_feats(x_set, './plot/x_set.jpg')
            x_dcn = self.dcn_pack(x, x_set)
            plot_feats(x_dcn, './plot/x_dcn.jpg')
            x_emb = self.emb_conv(x_dcn)
            plot_feats(x_emb, './plot/x_emb.jpg')
            x_cor = torch.sum(x_emb * x_emb[[i]].repeat(t, 1, 1, 1), dim=1, keepdim=True)
            x_cor = torch.sigmoid(x_cor)
            plot_feats(x_cor, './plot/x_cor.jpg')
            x_fus = x_cor.repeat(1, c, 1, 1) * x_ref.repeat(t, 1, 1, 1)
            plot_feats(x_fus, './plot/x_fus.jpg')
            x_fus = torch.sum(x_fus, dim=0, keepdim=True)
            plot_feats(x_fus, './plot/x_fus_f.jpg')
            feat.append(x_fus)
        out = torch.cat(feat, dim=0)
        plot_feats(x, './plot/output.jpg')
        return out
...
def plot_feats(x, path=''):
    with torch.no_grad():
        feats = np.asarray(x.cpu(), dtype=np.float32)
        feats_ = feats.sum(axis=1, keepdims=True)
        ts = feats.shape[0]
        fig = plt.figure(figsize=(10, ts*4+2))
        for t in range(ts):
            one = plt.subplot2grid((ts, 2), (t, 0))
            one.set_title(f't{t}_first_channel')
            _one = one.imshow(feats[t, 0, ...])

            sum = plt.subplot2grid((ts, 2), (t, 1))
            sum.set_title(f't{t}_sum_channels')
            _sum = sum.imshow(feats_[t, 0, ...])

            fig.colorbar(_sum, ax=[one, sum])
