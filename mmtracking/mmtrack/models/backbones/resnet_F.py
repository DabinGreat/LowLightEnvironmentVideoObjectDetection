...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = build_conv_layer(
            ...
            bias=False)
        self.conv1_1x1 = build_conv_layer(
            conv_cfg,
            planes,
            planes,
            kernel_size=1,
        ...
        if not self.with_dcn or fallback_on_stride:
            ...
            self.conv2 = build_conv_layer(
                ...
                dilation=dilation,
                bias=False)
        self.conv2_1x1 = build_conv_layer(
            conv_cfg,
            planes,
            planes,
            kernel_size=1,
        ...
        self.conv3 = build_conv_layer(
            ...
            kernel_size=1,
            bias=False)
        self.conv3_1x1 = build_conv_layer(
            conv_cfg,
            planes * self.expansion,
            planes * self.expansion,
            kernel_size=1,
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            identity = x
            out = self.conv1(x)
            out_ = self.conv1_1x1(out)
            out = self.norm1(out + out_)
            out = self.relu(out)
            ...
            out = self.conv2(out)
            out_ = self.conv2_1x1(out)
            out = self.norm2(out + out_)
            out = self.relu(out)
            ...
            out = self.conv3(out)
            out_ = self.conv3_1x1(out)
            out = self.norm3(out + out_)
        ...
        out_ = self.conv1x1(out)
        out = self.relu(out + out_)
