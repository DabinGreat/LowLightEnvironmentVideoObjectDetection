...
from mmcv.image import imread, imwrite, tensor2imgs
...
class SelsaFastDVDnetDetect(BaseVideoDetector):
    ...
    def __init__(self,
        ...
        super(SelsaFastDVDnetDetect, self).__init__()
    ...
    def forward_train(self,
        ...
        assert ref_img.shape[1] == 4, 'FastDVDnet support frames is 4.'
        ...
        clean_imgs = torch.cat([clean_ref[0][:2], clean_img, clean_ref[0][2:]], dim=0)
        noise_imgs = torch.cat([noise_ref[0][:2], noise_img, noise_ref[0][2:]], dim=0)

        noise2dvd = []
        for i in range(5):
            cat_list = [noise_imgs[[(i+j) % 5]] for j in [-2, -1, 0, 1, 2]]
            noise2dvd.append(torch.cat(cat_list, dim=1))
        noise2dvd = torch.cat(noise2dvd, dim=0)  # dim -> 5 x (5 x 3) x w x h

        denoising_imgs = self.cleaner(noise2dvd)  # to FastDVDnet

        # plot_img(denoising_imgs, img_metas)
        # plot_img(noise2dvd[:, 6:9, ...], img_metas)
        # plot_img(clean_imgs, img_metas)
        # plot_img(noise_imgs, img_metas)

        _, all_x = self.detector.extract_feat(denoising_imgs)
        x = []
        ref_x = []
        for i in range(len(all_x)):
            x.append(all_x[i][[2]])
            ref_x_ = torch.cat([all_x[i][:2], all_x[i][3:]], dim=0)
            ref_x.append(ref_x_)
        ...
        if self.with_cleaner:
            denoising_losses = dict()
            if self.loss_type == 'l1':
                denoising_loss = nn.L1Loss()
            elif self.loss_type == 'l2':
                denoising_loss = nn.MSELoss()
            elif self.loss_type == 'smooth_l1':
                denoising_loss = nn.SmoothL1Loss()
            else:
                raise NotImplementedError()

            denoising_losses[f'loss_denoising_{self.loss_type}'] = denoising_loss(denoising_imgs, clean_imgs)
    ...
    def extract_feats(self, img, img_metas, ref_img, ref_img_metas):
        ...
        if frame_stride < 1:
            if frame_id == 0:
                self.memo = Dict()
                self.memo.img_metas = ref_img_metas[0]
                self.memo.ref_imgs = ref_img[0]

            refs = self.memo.ref_imgs
            noise_imgs = torch.cat([refs[:2], img, refs[2:]], dim=0)
            ...
            denoising_imgs = self.cleaner(noise2dvd)  # to FastDVDnet
            _, all_x = self.detector.extract_feat(denoising_imgs)
            self.memo.denoising_img = denoising_imgs[[2]]

            x = []
            ref_x = []
            for i in range(len(all_x)):
                x.append(all_x[i][[2]])
                ref_x_ = torch.cat([all_x[i][:2], all_x[i][3:], all_x[i][[2]]], dim=0)
                ref_x.append(ref_x_)
            ref_img_metas = self.memo.img_metas.copy()
            ref_img_metas.extend(img_metas)
    ...
    def simple_test(self,
        ...
        plot_bbox = True
        if plot_bbox:
            out_dir = './result_fastdvdnet_out'
            out_file = os.path.join(out_dir, img_metas[0]['ori_filename'])
            h, w, _ = img_metas[0]['img_shape']
            # img_show = self.memo.denoising_img[..., :h, :w]
            img_show = img[..., :h, :w]
            img = tensor2imgs(img_show, **img_metas[0]['img_norm_cfg'])
            img = np.clip(img[0], 0, 255.)
            for i in range(len(results['bbox_results'])):
                scale_bbox = results['bbox_results'][i][:, :4] * img_metas[0]['scale_factor'][0]
                score_bbox = results['bbox_results'][i][:, [4]]
                results['bbox_results'][i] = np.concatenate([scale_bbox, score_bbox], axis=1)
            self.show_result(img, results, out_file=out_file)
...
def plot_img(img_tensor, img_metas):
    import matplotlib.pyplot as plt

    img = tensor2imgs(img_tensor, **img_metas[0]['img_norm_cfg'])
    plt.imshow(img[0][..., ::-1])
    plt.show()