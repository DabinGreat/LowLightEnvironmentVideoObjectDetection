...
class FGFA(BaseVideoDetector):
    ...
    def forward_train(self,
        ...
        # add noise
        ref_before = torch.cat([ref_img[i] for i in range(ref_img.size(0))], dim=0)
        all_before = torch.cat([img, ref_before], dim=0)
        noise, clean = general_clean_noise_pairs(all_before, constant=[0.5, 0.5])
        img = noise[[0]]
        ref_img = torch.stack(torch.chunk(noise[1:], ref_img.size(0), dim=0), dim=0)
