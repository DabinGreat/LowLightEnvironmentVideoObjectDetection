...
from mmtrack.core.utils import add_noise_clean_pairs, general_clean_noise_pairs


@MODELS.register_module()
class SelsaNewVIDDetect(BaseVideoDetector):
    ...
    def __init__(self,
        ...
        super(SelsaNewVIDDetect, self).__init__()
    ...
    def forward_train(self,
        ...
        noise, clean = add_noise_clean_pairs(all_imgs, **self.noise_cfg)
        ...
        if self.with_cleaner:
            ...
            for i in range(len(x_noise)):
                mse_losses[f'loss_mse_{i}_u'] = F.mse_loss(x_noise[i], x_clean[i])
                mse_losses[f'loss_mse_{i}_d'] = F.mse_loss(_x_noise[i], x_clean[i])
    ...
    def extract_feats(self, img, img_metas, ref_img, ref_img_metas):
        ...
        #         self.memo.ref_img = ref_img[0]
        #
        #     ref_img = self.memo.ref_img
        #     imgs = torch.cat([ref_img, img], dim=0)
        #     noise, clean = add_noise_clean_pairs(imgs, noise_type=self.noise_type)
        #     _, xs = self.detector.extract_feat(noise)
        #     x, ref_x = [], []
        #     for i in range(len(xs)):
        #         x.append(xs[i][[-1]])
        #         ref_x.append(xs[i])
        #     ref_img_metas = self.memo.img_metas.copy()
        #     ref_img_metas.extend

        if frame_stride < 1:
            if frame_id == 0:
                self.memo = Dict()
                self.memo.img_metas = ref_img_metas[0]
                noise, clean = add_noise_clean_pairs(ref_img[0], **self.noise_cfg)
                ref_noise_x, ref_x = self.detector.extract_feat(noise)
            ...
            noise, clean = add_noise_clean_pairs(img, **self.noise_cfg)
            noise_x, x = self.detector.extract_feat(noise)
        ...
        # test with fixed stride
            ...
            if frame_id == 0:
                ...
                self.memo.img = noise
                self.memo.img_metas = ref_img_metas[0]
                _, ref_x = self.detector.extract_feat(noise)
                # 'tuple' object (e.g. the output of FPN) does not support
                # item assignment
                self.memo.feats = []
                ...
                noise, clean = add_noise_clean_pairs(ref_img[0], **self.noise_cfg)
                x = []
                ref_imgs = torch.cat([self.memo.img, noise], dim=0)[1:]
                ...
                noise, clean = add_noise_clean_pairs(img, **self.noise_cfg)
                _, x = self.detector.extract_feat(noise)
    ...
    #             self.memo = Dict()
    #             self.memo.img_metas = ref_img_metas[0]
    #             noise, clean = general_clean_noise_pairs(ref_img[0], constant=[0.5, 0.5])
    #             _, ref_x = self.detector.extract_feat(clean)
    #             # 'tuple' object (e.g. the output of FPN) does not support
    #             # item assignment
    #             self.memo.feats = []
    #             for i in range(len(ref_x)):
    #                 self.memo.feats.append(ref_x[i])
    #
    #         noise, clean = general_clean_noise_pairs(img, constant=[0.5, 0.5])
    ...
    #             x = []
    #             ref_imgs = torch.cat([self.memo.img, noise], dim=0)[1:]
    #             # ref_imgs = torch.cat([self.memo.img, clean], dim=0)[1:]
    #             _, ref_x = self.detector.extract_feat(ref_imgs)
    #             for i in range(len(ref_x)):
    #                 self.memo.feats[i] = ref_x[i]
    #                 x.append(self.memo.feats[i][[num_left_ref_imgs]])
